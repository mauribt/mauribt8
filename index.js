...
pone ${prefix}verify para poder usar el bot.`
	...
	only: {
	  ...
	  ownerG: 'este comando solo puede ser usado por el creador del grupo',
  ...
  */
  ...
  case 'menu':
  hasil = `        ────────────────
  *${pushname}* usa este comando ${prefix}menu
		  ────────────────`
  reply(hasil)
		  break
  }
  ...
  *〘 TEXTO2 〙*
  ...
  ┃ _Ex : ${prefix}Ephoto 25 mauri_
  ┠⊷️ *${prefix}Textpro list*
  ┃ _Lista de textpro
  ┠⊷️ *${prefix}Textpro (code + Texto)*
  ┃ _Ex : ${prefix}Textpro 55 mauri_
  ┗━━━━━━━━━━━━━━━━━━━━
  *〘 IMAGENES 〙*
  ┏━━━━━━━━━━━━━━━━━━━━
  ┠⊷️ *${prefix}Cogan*
  ...
  */
  ...
  //--arti nama
  case 'arti':
  if (args.length < 1) return reply('*☒* manda el texto gordito')
  Lxa.updatePresence(from, Presence.composing)
  if (!isRegister) return reply(mess.only.daftarB)
  teks = body.slice(6)
  try {
  ...
  //---Zodiak harian
  case 'zodiak':
  if (args.length < 1) return reply('y el signo? q pelotudo q sos')
  Lxa.updatePresence(from, Presence.composing)
  if (!isRegister) return reply(mess.only.daftarB)
  teks = body.slice(8)
  try {
  ...
  if (args.length < 1) return reply('pone lo que queres buscar mogolico')
  tels = body.slice(5)
  Lxa.updatePresence(from, Presence.composing)
  reply(mess.wait)
  try {
  ...
  if (args.length < 1) return reply('pone lo q queres buscar')
  Lxa.updatePresence(from, Presence.composing)
  try {
  data = await fetchJson(`https://api.fdci.se/sosmed/rep.php?gambar=${tels}`, {
	method: 'get'
  ...
  case 'trolo':
	...
	if (args.length < 1) return Lxa.sendMessage(from, 'pone el nombre pelotudo', text, {
  quoted: mek
	})
	teks = body.slice(13)
	Lxa.updatePresence(from, Presence.composing)
  ...
  case 'furry':
	...
	if (args.length < 1) return Lxa.sendMessage(from, 'pone el nombre junto al comando', text, {
  quoted: mek
	})
	teks = body.slice(15)
	try {
  ...
  //--searching lirik
  case 'lirik':
	if (args.length < 1) return reply('pone el nombre de la cancion')
	if (!isRegister) return reply(mess.only.daftarB)
  ...
  case 'alay':
	if (args.length < 1) return reply('y el texto ? pelotudo')
  Lxa.updatePresence(from, Presence.composing)
  if (!isRegister) return reply(mess.only.daftarB)
  ...
  Lxa.sendMessage(from, `@${mentioned[0].split('@')[0]} volves a hacer eso y te vas kickeado gordito`, text, {
	quoted: mek
  })
  break
  ...
  Lxa.sendMessage(from, 'cambio el nombre del grupo', text, {
	quoted: mek
  })
  break
  ...
  Lxa.sendMessage(from, 'cambio la desc. del grupo', text, {
	quoted: mek
  })
  break
  ...
  */
  ...
  ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
	...
	‣ link de wa.me : wa.me/${sender.split("@")[0]}`
  ...
  case 'fb':
	...
	if (anu.error) return reply(anu.error)
	infomp3 = `*Video*\n‣ *Nombre* : ${anu.title}\n‣ *Publicación* : ${anu.published}\n‣ *Tamaño* : ${anu.filesize}\n\n_El video se esta enviando, si no llega descarga por el link_\n‣ *link* : ${anu.result}`
	lagu = await getBuffer(anu.result)
	Lxa.sendMessage(from, lagu, video, {
  mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: mek
	})
  ...
  */
  ...
  //--block user
				  case 'blocklist':
					  teks = 'lista de bloqueados del bot :\n'
  ...
  //---textpro
				  case 'textpro':
					  if (args.length < 1) {
						  ...
						  return reply('escribi bien pelotudo')
					  }
					  reply(mess.wait)
					  ...
					  if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						  ...
						  await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							  ...
							  exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								  fs.unlinkSync(ranp)
								  if (err) return reply(mess.error.stick)
								  exec(`webpmux -set exif ${addMetadata('𝘮𝘢𝘶𝘳𝘪', 'bot')} ${ranw} -o ${ranw}`, async (error) => {
									  if (error) return reply(mess.error.stick)
  ...
  exec(`ffmpeg -i ${anum} ${ran}`, (err) => {
	fs.unlinkSync(anum)
	if (err) return reply('no se pudo convertir el video a sticker')
	buffer = fs.readFileSync(ran)
	Lxa.sendMessage(from, buffer, video, {
  quoted: mek, caption: '𝘮𝘢𝘶𝘳𝘪'
	})
	fs.unlinkSync(ran)
  })
  break
  ...
  //--google voice
				  case 'tts':
					  ...
					  if (args.length < 2) return Lxa.sendMessage(from, 'escribi el texto', text, {quoted: mek})
					  dtt = body.slice(8)
					  ranm = getRandom('.mp3')
					  dtt.length > 600
					  ? reply('escribi bien pelotudo')
  ...
  //--menaikan jabatan
		case 'promote':
					  ...
					  if (mentioned.length > 1) {
						  ...
						  mentions(`ahora @${mentioned[0].split('@')[0]} es admin`, mentioned, true)
  ...
  Lxa.groupUpdateSubject(idgrup, `${body.slice(9)}`)
  Lxa.sendMessage(from, 'cambia el nombre del grupo', text, {
	quoted: mek
  })
  break
  ...
  if (!isBotGroupAdmins) return reply(mess.only.Badmin)
  Lxa.groupUpdateDescription(from, `${body.slice(9)}`)
  Lxa.sendMessage(from, 'cambia la descripción del grupo', text, {
	quoted: mek
  })
  break
  ...
  //--menurunkan jabatan
				  case 'demote':
					  ...
					  if (mentioned.length > 1) {
						  ...
						  mentions(`jaja por maricon@${mentioned[0].split('@')[0]}`, mentioned, true)
  ...
  //--mengeluarkan member
				  case 'kick':
					  ...
					  if (mentioned.length > 1) {
						  ...
						  mentions(`un gordo de mierda menos @${mentioned[0].split('@')[0]}`, mentioned, true)
						  Lxa.groupRemove(from, mentioned)
					  }
					  break
  ...
  //--list admin grup
				  case 'listadmins':
					case 'listadmin':
					  case 'adminlist':
					  if (!isGroup) return reply(mess.only.group)
					  teks = `admins: *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					  no = 0
  ...
  */
  ...
  //--Convert stiker to image
				  case 'toimg':
					  ...
					  exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						  ...
						  buffer = fs.readFileSync(ran)
						  Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: '𝘮𝘢𝘶𝘳𝘪-bt'})
						  fs.unlinkSync(ran)
					  })
					  break
  ...
  case 'daftar':
	case 'register':
  if (isRegister) return reply('ya estabas verificado pelotudo')
  ...
  tm = `usa ${prefix}menu para ver la lista de comandos gordito puto`
  reply(tm)
  break
  ...
  Lxa.deleteMessage(from, {
	...
	reply('solo puedo borrar mensajes mios boludito')
  }
  break
  ...
  break
				  ...
				  case 'welcome':
					  ...
					  if (Number(args[0]) === 1) {
						  if (isWelkom) return reply('ya esta activo hijo de puta')
						  welkom.push(from)
						  ...
						  reply('".welcome 1" para activar "welcome 0" para desactivar')
					  }
										break
				  case 'clone':
					  if (!isGroup) return reply(mess.only.group)
